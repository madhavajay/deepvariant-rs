                    ...
                    self.state = match feature {
                        ...
                        Feature::Substitution { code, .. } => {
                            if let Some(reference_sequence) = self.reference_sequence {
                                ...
                                // Local patch (matches upstream noodles-cram
                                // 0.93.0): a non-ACGT reference base (N in
                                // GRCh38 centromeres/gaps/decoy) must not
                                // panic — fall back to N, as htslib does.
                                // Without this, real WGS CRAMs abort with
                                // `invalid reference base` whenever a read
                                // carries a Substitution feature over an N.
                                let reference_base =
